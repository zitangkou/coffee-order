...
import { cancelStaleOrders, restoreScheduledProducts } from "./services/order.js";
...
restoreScheduledProducts().catch((e) => console.error("[cron] 首次售罄恢复失败", e));
setInterval(() => {
  cancelStaleOrders().catch((e) => console.error("[cron] 自动取消失败", e));
  syncProcessingWechatRefunds().catch((e) => console.error("[cron] 退款状态同步失败", e));
  restoreScheduledProducts().catch((e) => console.error("[cron] 售罄恢复失败", e));
}, 5 * 60 * 1000);
